          ...
          {recentWorklogs.map((worklog) => (
            ...
            >
              ...
              <div className="flex-1 space-y-1">
                <p className="text-sm font-medium leading-none">{worklog.title}</p>
                <p className="text-xs text-muted-foreground">{worklog.project}</p>
              </div>
              <div className="flex items-center gap-1 text-xs text-muted-foreground">
                <Clock className="h-3 w-3" />
                <span>{worklog.hours}h</span>
              </div>
              <span className="text-xs text-muted-foreground">{worklog.date}</span>
            </div>
          ))}
        </div>
      </CardContent>
